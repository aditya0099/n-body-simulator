...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* constructTree(vector<Point<Dim> > points, int lo, int hi, int d);

    /** Selects the m-th smallest point from a vector of points */
    int quickSelect(int lo, int hi, int d, vector<Point<Dim> > &data, int m);

    /** Helper function for quickselect, partitions the data based on the last element */
    int partition(vector<Point<Dim> > &data, int lo, int hi, int d);

    /** Recurisve helper function to find the nearest neighbor point to query */
    typename KDTree<Dim>::KDTreeNode* findNearestNeighbor(const Point<Dim>& query, int d, KDTreeNode* subroot) const;

    /** Helper function to calculate the squared euclidian distnace between two points */
    double getDistSquared(const Point<Dim>& p1, const Point<Dim>& p2) const;

    /** Copies another tree into the current tree */
    void _copy(const KDTree& other);

    /** Recursive helper function for copying the tree */
    KDTreeNode* _copyTree(KDTreeNode* subroot);

    /** Resursive helper function to delete all nodes of a tree */
    void _clear(KDTreeNode* subroot);
};
